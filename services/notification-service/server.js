    ...
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        if (event.type === "TransactionCompleted") {
          console.log(
            "Received TransactionCompleted event:",
          ...
          await NotificationService.sendSMS(
            "+251900478653",
            `Transaction ${event.payload.transaction_id} of ${event.payload.amount} ${event.payload.currency} completed. Receipt: ${receiptUrl}`
          );
        } else if (event.type === "UserRegistered") {
          console.log("Received UserRegistered event:", event.payload.user_id);
          await NotificationService.sendSMS(
            "+251900478653",
            `Welcome ${event.payload.username}! Thanks for registering with UPOP.`
          );
        }
      } catch (error) {
